...
**重要澄清：`approveBuilderFee` 是用户操作，不是 Builder 操作。**
...
│  ✅ 白名单审核                   不需要      无审核流程          │
│  ─────────────────────────────────────────────────────────────  │
│  ⚠️  Perps 账户余额 >= 100 USDC  必须满足    唯一硬性条件        │
│  ─────────────────────────────────────────────────────────────  │
│                                                                  │
│  满足上述条件后，该地址即可作为 Builder 接收费用                 │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
// 响应
{
  "unclaimedRewards": "12.34",    // 未领取的收益
  "claimedRewards": "100.00",     // 已领取的收益
  "builderRewards": "50.00",      // Builder 费用收益
  // ... 其他字段
}
```
...
```
URL: https://stats-data.hyperliquid.xyz/Mainnet/builder_fills/{builder_address}/{YYYYMMDD}.csv.lz4

注意：
- builder_address 必须全部小写
- 日期格式：YYYYMMDD (如 20250121)
- 文件格式：LZ4 压缩的 CSV
```
