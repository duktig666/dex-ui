...
export function useTrading() {
  ...
  const { builderFeeApproved, setBuilderFeeApproved, updateOpenOrders, updateClearinghouseState } =
    useUserStore();
  ...
  const placeOrder = useCallback(
    async (params: PlaceOrderParams): Promise<TradeResult> => {
      const {
        coin,
        side,
        orderType,
  ...
  const cancelOrder = useCallback(
    async (params: CancelOrderParams): Promise<TradeResult> => {
      ...
      try {
        ...
        const response = await exchangeClient.cancelOrder([{ assetId, oid }], signTypedData);
  ...
  const cancelAllOrders = useCallback(
    async (coin?: string): Promise<TradeResult> => {
      if (!isConnected || !address) {
        return { success: false, error: 'Please connect wallet' };
      }
      ...
      try {
        const orders = await infoClient.getOpenOrders(address);
        const ordersToCancel = coin ? orders.filter((o) => o.coin === coin) : orders;
  ...
  const updateLeverage = useCallback(
    async (coin: string, leverage: number, isCross: boolean): Promise<TradeResult> => {
      if (!isConnected || !address) {
        return { success: false, error: 'Please connect wallet' };
      }
  ...
  const closePosition = useCallback(
    async (coin: string, slippagePercent: number = 1): Promise<TradeResult> => {
      if (!isConnected || !address) {
        return { success: false, error: 'Please connect wallet' };
      }
